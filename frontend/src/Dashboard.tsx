...
const Dashboard: React.FC = () => {
  ...
  const statusPieData = useMemo(() => {
    if (!metrics?.requests_by_status) return [];
    return Object.entries(metrics.requests_by_status).map(([status, count]) => ({
  ...
  const methodBarData = useMemo(() => {
    if (!metrics?.requests_by_method) return [];
    return Object.entries(metrics.requests_by_method).map(([method, count]) => ({
      method,
      count,
    }));
  }, [metrics]);
  ...
  const endpointColumns: ColumnsType<EndpointStat> = [
    { title: 'Endpoint', dataIndex: 'path', key: 'path', ellipsis: true, width: '35%' },
    { title: 'Requests', dataIndex: 'count', key: 'count', sorter: (a, b) => a.count - b.count, width: '13%' },
    { title: 'Errors', dataIndex: 'errors', key: 'errors', sorter: (a, b) => a.errors - b.errors, width: '13%' },
    { title: 'Avg (ms)', dataIndex: 'avg_response_time', key: 'avg_response_time', sorter: (a, b) => a.avg_response_time - b.avg_response_time, render: (t: number) => t.toFixed(2), width: '13%' },
    { title: 'P95 (ms)', dataIndex: 'p95_response_time', key: 'p95_response_time', sorter: (a, b) => a.p95_response_time - b.p95_response_time, render: (t: number) => t.toFixed(2), width: '13%' },
    {
      title: 'Error Rate', dataIndex: 'error_rate', key: 'error_rate',
      render: (rate: number) => (
        <Tag color={rate > 5 ? 'red' : rate > 1 ? 'orange' : 'green'}>{rate.toFixed(2)}%</Tag>
      ),
  ...
  const errorColumns: ColumnsType<ErrorLog> = [
    { title: 'Time', dataIndex: 'timestamp', key: 'timestamp', render: (ts: string) => new Date(ts).toLocaleString(), width: 180 },
    ...
    { title: 'Path', dataIndex: 'path', key: 'path', ellipsis: true },
    {
      ...
      render: (status?: number) => (status ? <Tag color={status >= 500 ? 'red' : 'orange'}>{status}</Tag> : '-'),
      width: 80,
    },
    { title: 'Error', dataIndex: 'error_message', key: 'error_message', ellipsis: true, render: (msg?: string, record?: ErrorLog) => msg || record?.error_type || '-' },
  ];
  ...
  const renderContent = () => {
    ...
    switch (activePage) {
          ...
          <>
            {healthStatus && !healthStatus.log_file.exists && (
              <Alert
                ...
                description={`Cannot find monitoring log at: ${healthStatus.log_file.path}.`}
                type="error"
                showIcon
            ...
            {healthStatus?.latest_timestamp && (
              <Alert
                message="Log Time Anchor"
                description={
                  <div>
                    <div>Time windows are computed relative to the latest log timestamp.</div>
                    <Text type="secondary">Latest log: {new Date(healthStatus.latest_timestamp).toLocaleString()}</Text>
                  </div>
                }
                type="info"
                showIcon
            ...
            <Row gutter={[16, 16]}>
              <Col xs={24} sm={12} lg={6}>
                <Card>
                  <Statistic title="Total Requests" value={metrics?.total_requests || 0} valueStyle={{ color: '#1890ff' }} prefix={<CheckCircleOutlined />} />
                </Card>
              </Col>
              <Col xs={24} sm={12} lg={6}>
                <Card>
                  <Statistic title="Avg Response Time" value={metrics?.avg_response_time || 0} precision={2} suffix="ms" valueStyle={{ color: '#52c41a' }} />
                </Card>
              </Col>
              <Col xs={24} sm={12} lg={6}>
                <Card>
              ...
              <Col xs={24} sm={12} lg={6}>
                <Card>
                  <Statistic title="P95 Latency" value={metrics?.p95_response_time || 0} precision={2} suffix="ms" valueStyle={{ color: '#722ed1' }} />
                </Card>
              </Col>
            </Row>
            ...
            <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
              <Col xs={24} sm={12} lg={6}>
                <Card>
                  <Statistic title="Unique Users" value={metrics?.unique_users || 0} prefix={<UserOutlined />} />
                </Card>
              </Col>
              <Col xs={24} sm={12} lg={6}>
                <Card>
                  <Statistic title="Authenticated Requests" value={metrics?.authenticated_requests || 0} />
                </Card>
              </Col>
              <Col xs={24} sm={12} lg={6}>
                <Card>
                  <Statistic title="P50 Latency" value={metrics?.p50_response_time || 0} precision={2} suffix="ms" />
                </Card>
              </Col>
              <Col xs={24} sm={12} lg={6}>
                <Card>
                  <Statistic title="P99 Latency" value={metrics?.p99_response_time || 0} precision={2} suffix="ms" />
                </Card>
              </Col>
            </Row>
                ...
                <Card title="HTTP Status Codes Distribution">
                  {statusPieData.length > 0 ? (
                    <Pie
                      ...
                      colorField="type"
                      radius={0.8}
                      // ✅ FIX: no label type 'outer' (your version doesn't support it)
                      label={false}
                      tooltip={{
                        formatter: (datum: any) => ({
                          name: datum.type,
      ...
      case 'api-performance':
        return (
          <>
            <Card title="API Endpoint Performance" extra={<Text type="secondary">Top 10 endpoints by request count</Text>}>
              {endpoints.length > 0 ? (
                <Table
            ...
            {trafficData.length > 0 && (
              <Card title="Traffic Summary" style={{ marginTop: 24 }}>
                <Row gutter={16}>
                  ...
                  <Col span={8}>
                    <Statistic title="Peak Requests" value={Math.max(...trafficData.map((d) => d.requests))} />
                  </Col>
                  <Col span={8}>
                    <Statistic title="Total Requests (24h)" value={trafficData.reduce((sum, item) => sum + item.requests, 0)} />
                  </Col>
                </Row>
              </Card>
            )}
              ...
              <Card title="Hourly Breakdown" style={{ marginTop: 24 }}>
                <Table
                  ...
                  columns={[
                    ...
                    { title: 'Requests', dataIndex: 'requests', key: 'requests', sorter: (a, b) => a.requests - b.requests },
                  ]}
                  pagination={false}
                  size="small"
                />
    ...
    <Layout style={{ minHeight: '100vh' }}>
      <Sider width={220} theme="light" breakpoint="lg" collapsedWidth="0">
        ...
        >
          ...
          <Menu.Item key="api-performance" icon={<ApiOutlined />}>API Performance</Menu.Item>
          <Menu.Item key="errors" icon={<WarningOutlined />}>Errors & Exceptions</Menu.Item>
          <Menu.Item key="traffic" icon={<BarChartOutlined />}>Traffic Analysis</Menu.Item>
        </Menu>
      </Sider>
            ...
            {activePage === 'overview' ? 'Dashboard Overview'
              : activePage === 'api-performance' ? 'API Performance'
                : activePage === 'errors' ? 'Errors & Exceptions'
                  : activePage === 'traffic' ? 'Traffic Analysis'
